...
public class Site implements EntryPoint, ValueChangeHandler<String> {
    ...
    private static final SiteImageBundle images = (SiteImageBundle) GWT.create(SiteImageBundle.class);
    ...
    private void init() {
        ...
        DOM.setStyleAttribute(RootPanel.get().getElement(), "backgroundImage", "url(images/frdbg1b.gif)");
        DOM.setStyleAttribute(RootPanel.get().getElement(), "backgroundPosition", "top left");
        DOM.setStyleAttribute(RootPanel.get().getElement(), "backgroundRepeat", "repeat");

        mainPanel = new DockPanel();
        DOM.setStyleAttribute(mainPanel.getElement(), "backgroundImage", "url(images/fr1bg1a.gif)");
        DOM.setStyleAttribute(mainPanel.getElement(), "backgroundPosition", "top left");
        DOM.setStyleAttribute(mainPanel.getElement(), "backgroundRepeat", "repeat");
        ...
        mainPanel.setCellHorizontalAlignment(header, HorizontalPanel.ALIGN_CENTER);
        ...
        contentTopTile.setCellHorizontalAlignment(pageLabelTile, DockPanel.ALIGN_CENTER);
        ...
        poweredByTile.setCellVerticalAlignment(poweredByLabel, HorizontalPanel.ALIGN_MIDDLE);
        HTML poweredByLink = new HTML("<a href=\"http://www.graphicgarden.com/\" target=\"blank\"><img src=\"images/ggarden.gif\" border=\"0\"></a>");
        poweredByTile.add(poweredByLink);
        poweredByTile.setCellVerticalAlignment(poweredByTile, HorizontalPanel.ALIGN_MIDDLE);
        mainPanel.add(poweredByTile, DockPanel.SOUTH);
        mainPanel.setCellHorizontalAlignment(poweredByTile, HorizontalPanel.ALIGN_CENTER);
        mainPanel.setCellVerticalAlignment(poweredByTile, HorizontalPanel.ALIGN_BOTTOM);
        ...
        mainPanel.setCellHorizontalAlignment(footer, HorizontalPanel.ALIGN_CENTER);
            ...
            public void run() {
                try {
                    if (info != curInfo) {
                        ...
                        DOM.setStyleAttribute(pageLabel.getElement(), "fontSize", "24pt");
                        DOM.setStyleAttribute(pageLabel.getElement(), "fontWeight", "bold");
                        DOM.setStyleAttribute(pageLabel.getElement(), "color", "#859db6");
                        DOM.setStyleAttribute(pageLabel.getElement(), "marginTop", "10pt");
                        pageLabelTile.setWidget(pageLabel);
                        contentPanel.setWidget(curSink.getContent());
                        contentAdditionsPanel.setWidget(curSink.getContentAdditions());
    ...
    /**
     * Adds all sinks to the list. Note that this does not create actual instances of all
     * sinks yet (they are created on-demand). This can make a significant difference in
     * startup time.
     */
