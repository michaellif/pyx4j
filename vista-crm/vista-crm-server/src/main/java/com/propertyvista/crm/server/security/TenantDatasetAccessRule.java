...
public class TenantDatasetAccessRule implements DatasetAccessRule<Customer> {
    ...
    public void applyRule(EntityQueryCriteria<Customer> criteria) {
        ...
        criteria.or()
                ...
                .right(PropertyCriterion.eq(criteria.proto()._tenantInLease().$().leaseTermV().holder().lease().unit().building().userAccess(), Context
                        .getVisit().getUserVisit().getPrincipalPrimaryKey()))
