...
public class UnitForm extends CrmEntityForm<AptUnitDTO> {
	...
	public void createTabs() {
		...
		tab = addTab(isEditable() ? new HTML()
				: ((UnitViewerView) getParentView()).getUnitItemsListerView()
						.asWidget(), i18n.tr("Details"));
		setTabEnabled(tab, !isEditable());

		if (VistaFeatures.instance().occupancyModel()) {
			tab = addTab(isEditable() ? new HTML()
					: ((UnitViewerView) getParentView())
							.getOccupanciesListerView().asWidget(),
					i18n.tr("Occupancy"));
			setTabEnabled(tab, !isEditable());
		}
	...
	protected void onValueSet(boolean populate) {
		...
		if (VistaFeatures.instance().occupancyModel()) {
			get(proto()._availableForRent()).setVisible(
					!getValue()._availableForRent().isNull());
			get(proto().financial()._unitRent()).setVisible(
					!getValue().financial()._unitRent().isNull());
		}
	}
	...
	private FormFlexPanel createGeneralTab(String title) {
		...
		left.setWidget(
				...
				new DecoratorBuilder(
						inject(proto().building(),
								isEditable() ? new CEntityLabel<Building>()
										: new CEntityCrudHyperlink<Building>(
												AppPlaceEntityMapper
														.resolvePlace(Building.class))),
						20).build());

		left.setWidget(
				++row,
				0,
				new DecoratorBuilder(inject(proto().floorplan(),
						new FloorplanSelectorHyperlink()), 20).build());
		left.setWidget(++row, 0, new DecoratorBuilder(inject(proto().info()
				.economicStatus()), 20).build());
		left.setWidget(++row, 0, new DecoratorBuilder(inject(proto().info()
				.economicStatusDescription()), 20).build());

		left.setBR(++row, 0, 1);
		left.setWidget(
				++row,
				0,
				new DecoratorBuilder(inject(
						proto().lease(),
						isEditable() ? new CEntityLabel<Lease>()
								: new CEntityCrudHyperlink<Lease>(
										AppPlaceEntityMapper
												.resolvePlace(Lease.class))),
						20).build());
		left.setWidget(++row, 0, new DecoratorBuilder(inject(proto()
				._availableForRent()), 9).build());
		left.setWidget(++row, 0, new DecoratorBuilder(inject(proto()
				.financial()._unitRent()), 7).build());

		if (VistaFeatures.instance().productCatalog()) {
			left.setH3(++row, 0, 1, proto().marketPrices().getMeta()
					.getCaption());
			left.setWidget(
					++row,
					0,
					inject(proto().marketPrices(), new UnitServicePriceFolder()));
		} else {
			left.setWidget(++row, 0, new DecoratorBuilder(inject(proto()
					.financial()._marketRent()), 10).build());
		}

		row = -1;
		FormFlexPanel right = new FormFlexPanel();
		right.setWidget(++row, 1, new DecoratorBuilder(inject(proto().info()
				.floor()), 5).build());
		right.setWidget(++row, 1, new DecoratorBuilder(inject(proto().info()
				.number()), 5).build());
		...
		main.getFlexCellFormatter().setVerticalAlignment(0, 0,
	...
	private static class BuildingBoundFloorplanSelectorDialog extends
			EntitySelectorTableDialog<Floorplan> {
		...
		public BuildingBoundFloorplanSelectorDialog(Key ownerBuildingPk,
				AsyncCallback<Floorplan> onSelectedCallback) {
			super(Floorplan.class, false, Collections.<Floorplan> emptyList(),
					i18n.tr("Select Floorplan"));
		...
		protected AbstractListService<Floorplan> getSelectService() {
			return GWT
					.<SelectFloorplanListService> create(SelectFloorplanListService.class);
		}
	}

	private class FloorplanSelectorHyperlink extends
			CEntitySelectorHyperlink<Floorplan> {
			...
			addValueChangeHandler(new ValueChangeHandler<Floorplan>() {
				...
				public void onValueChange(ValueChangeEvent<Floorplan> event) {
					if (event.getValue() != null) {
						get(UnitForm.this.proto().info()._bedrooms()).setValue(
								event.getValue().bedrooms().getValue());
						get(UnitForm.this.proto().info()._bathrooms())
								.setValue(
										event.getValue().bathrooms().getValue());

						if (get(UnitForm.this.proto().info().area()).getValue() == null) {
							get(UnitForm.this.proto().info().area()).setValue(
									event.getValue().area().getValue());
							get(UnitForm.this.proto().info().areaUnits())
									.setValue(
											event.getValue().areaUnits()
													.getValue());
						}
					} else {
						get(UnitForm.this.proto().info()._bedrooms()).setValue(
		...
		protected AppPlace getTargetPlace() {
			return AppPlaceEntityMapper.resolvePlace(Floorplan.class,
					getValue().getPrimaryKey());
		}

		@Override
		protected AbstractEntitySelectorDialog<Floorplan> getSelectorDialog() {
			return new BuildingBoundFloorplanSelectorDialog(UnitForm.this
					.getValue().building().getPrimaryKey(),
					new DefaultAsyncCallback<Floorplan>() {
						@Override
						public void onSuccess(Floorplan result) {
							get(UnitForm.this.proto().floorplan()).setValue(
									result);
						}
					});
		}
