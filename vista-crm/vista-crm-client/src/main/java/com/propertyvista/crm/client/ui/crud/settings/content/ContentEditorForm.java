...
public class ContentEditorForm extends CrmEntityForm<ContentDescriptor> {

    private final VistaTabLayoutPanel tabPanel = new VistaTabLayoutPanel(VistaCrmTheme.defaultTabHeight, Unit.EM);

    public ContentEditorForm(IFormView<ContentDescriptor> parentView) {
        this(parentView, new CrmEditorsComponentFactory());
    }

    public ContentEditorForm(IFormView<ContentDescriptor> parentView, IEditableComponentFactory factory) {
        super(ContentDescriptor.class, factory);
                        ...
                        if (column.getObject().equals(proto().caption())) {
                            ...
                            if (parent.isEditable()) {
                                ...
                                comp = inject(column.getObject(), new CHyperlink(new Command() {
                                    @Override
                                    public void execute() {
                                        ((ContentEditor) getParentView()).viewChild(getValue().getPrimaryKey());
                                    }
                                }));
                            }
                            return comp;
                ...
                decor.addItemAddClickHandler(new ClickHandler() {
                    ...
                    public void onClick(ClickEvent event) {
                        if (ContentEditorForm.this.getValue().getPrimaryKey() != null) { // parent shouldn't be new unsaved value!..
                            ((ContentViewer) getParentView()).editNew(ContentEditorForm.this.getValue().getPrimaryKey());
                        }
                    }
                });
