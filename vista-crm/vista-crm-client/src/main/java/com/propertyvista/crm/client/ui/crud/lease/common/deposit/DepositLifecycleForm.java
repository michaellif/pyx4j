...
public class DepositLifecycleForm extends CrmEntityForm<DepositLifecycleDTO> {
    ...
    public DepositLifecycleForm(IForm<DepositLifecycleDTO> view) {
        ...
        int row = -1;
        content.setWidget(++row, 0, 2, inject(proto().deposit().billableItem(), new FieldDecoratorBuilder(true).build()));

        content.setWidget(++row, 0, inject(proto().deposit().type(), new FieldDecoratorBuilder(12).build()));
        content.setWidget(row, 1, inject(proto().status(), new FieldDecoratorBuilder(9).build()));

        content.setWidget(++row, 0, inject(proto().deposit().amount(), new FieldDecoratorBuilder(7).build()));
        content.setWidget(row, 1, inject(proto().currentAmount(), new FieldDecoratorBuilder(7).build()));

        content.setWidget(++row, 0, inject(proto().depositDate(), new FieldDecoratorBuilder(9).build()));
        content.setWidget(row, 1, inject(proto().refundDate(), new FieldDecoratorBuilder(9).build()));

        content.setWidget(++row, 0, 2, inject(proto().deposit().description(), new FieldDecoratorBuilder(true).build()));
