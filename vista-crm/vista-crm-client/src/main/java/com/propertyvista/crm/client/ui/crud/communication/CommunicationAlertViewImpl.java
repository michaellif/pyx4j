...
public class CommunicationAlertViewImpl extends FlowPanel implements CommunicationAlertView, RequiresResize {
    ...
    public CommunicationAlertViewImpl() {
        ...
        ticketsAnchor = new Anchor(i18n.tr("Dispatch Queue"), new Command() {
            ...
            public void execute() {
                ...
                place.queryArg(CommunicationThreadDTO.ViewScope.class.getSimpleName(), CommunicationThreadDTO.ViewScope.DispatchQueue.toString());
    ...
    @Override
    public void populate(Vector<CommunicationThreadDTO> messages) {
        mainHolder.clear();
        int directMessagesNum = 0;
        int dispatchedMessagesNum = 0;
        if (messages != null && messages.size() > 0) {
            mainHolder.insert(messageHeaderHolder, 0);
            for (final CommunicationThreadDTO message : messages) {
    ...
    class MessagePanel extends FlexTable {
        ...
        public MessagePanel(final CommunicationThreadDTO message, boolean isDirect) {
            ...
            dateField = new Label(message.representingMessage().date().getStringView());
            senderField = new Label(message.representingMessage().senderDTO().getStringView());

            if (message.representingMessage().highImportance().getValue(false)) {
