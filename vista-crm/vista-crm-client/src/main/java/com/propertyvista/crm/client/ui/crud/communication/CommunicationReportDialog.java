...
public class CommunicationReportDialog extends AbstractVisorController {
    private static final I18n i18n = I18n.get(CommunicationReportDialog.class);

    private final VisorCommunicationLister lister;

    private final AbstractVisorPaneView entityListVisorView;

    public CommunicationReportDialog(IPrimeViewerView<?> parentView, List<LeaseParticipant<?>> recipientScope) {
        super(parentView);

        lister = new VisorCommunicationLister(parentView.getPresenter(), recipientScope);
    ...
    protected Collection<CommunicationThreadDTO> getSelectedItems() {
        return lister.getSelectedItems();
    }

    protected CommunicationThreadDTO getSelectedItem() {
