...
public class CommunicationEditForm extends CrmEntityForm<CommunicationThreadDTO> {

    private static final I18n i18n = I18n.get(CommunicationEditForm.class);

    CommunicationEndpointSelector epSelectorNew;

    public CommunicationEditForm(IPrimeFormView<CommunicationThreadDTO, ?> view) {
    ...
    public IsWidget createGeneralForm() {
        ...
        formPanel.append(Location.Dual, proto().representingMessage().to(), epSelectorNew = createCommunicationEndpointSelectorNew()).decorate();
        ((CSelectorListBox) get(proto().representingMessage().to())).setWatermark(i18n.tr("+Add more"));

        inject(proto().deliveryMethod());

        formPanel.append(Location.Left, proto().representingMessage().onBehalf(), new TenantSelector()).decorate();
        formPanel.append(Location.Right, proto().representingMessage().onBehalfVisible()).decorate().customLabel(i18n.tr("Is Visible For Tenant"));
        formPanel.append(Location.Left, proto().allowedReply()).decorate();
        formPanel.append(Location.Right, proto().representingMessage().highImportance()).decorate();
    ...
    private void setVisibility(boolean isVisible, boolean isForNotification) {
        ...
        get(proto().representingMessage().onBehalf()).setVisible(isVisible);
        get(proto().representingMessage().onBehalfVisible()).setVisible(isVisible);
        get(proto().representingMessage().highImportance()).setVisible(isVisible);
    ...
    public void addToItem(CommunicationEndpointDTO item) {
        getValue().representingMessage().to().add(item);
    }

    public void removeToItem(CommunicationEndpointDTO item) {
        getValue().representingMessage().to().remove(item);
    }
