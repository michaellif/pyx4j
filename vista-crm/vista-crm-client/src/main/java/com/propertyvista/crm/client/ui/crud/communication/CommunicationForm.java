...
public class CommunicationForm extends CrmEntityForm<CommunicationThreadDTO> {

    private static final I18n i18n = I18n.get(CommunicationForm.class);

    private final MessageFolder messagesFolder;

    public CommunicationForm(IPrimeFormView<CommunicationThreadDTO, ?> view) {
        super(CommunicationThreadDTO.class, view);
    ...
    public void assignOwnership(IEntity employee, String additionalComment) {
        ((CommunicationViewerView.Presenter) getParentView().getPresenter()).assignOwnership(getValue(), additionalComment, employee);
    }

    public void hideUnhide() {
        ((CommunicationViewerView.Presenter) getParentView().getPresenter()).hideUnhide(getValue());
    }
    ...
    public IsWidget createGeneralForm() {
        ...
        formPanel.append(Location.Left, proto().id()).decorate();
        formPanel.br();
        ...
        formPanel.append(Location.Left, proto().representingMessage().header()).decorate().customLabel(i18n.tr("Sent"));
        formPanel.append(Location.Dual, proto().representingMessage().to(), new CommunicationEndpointSelector()).decorate();
    ...
    protected void onValueSet(boolean populate) {
        ...
        if (getValue() == null || getValue().isPrototype()) {
            ...
            get(proto().childMessages()).setVisible(true);
            ...
            get(proto().id()).setVisible(CategoryType.Ticket.equals(getValue().category().categoryType().getValue()));
            ...
            get(proto().childMessages()).setVisible(isEmpty || isDeliveryMethodEmpty);
            ...
            get(proto().representingMessage().header()).setVisible(!isEmpty && !isDeliveryMethodEmpty);
            get(proto().representingMessage().to()).setVisible(!isEmpty && !isDeliveryMethodEmpty);
    ...
    public static class MessageFolderItem extends CForm<MessageDTO> {
        ...
        public IsWidget createContent() {
            ...
            starImage.addClickHandler(new ClickHandler() {
                ...
                public void onClick(ClickEvent event) {
                    ...
                    ((CommunicationViewerActivity) parentForm.getParentView().getPresenter()).saveMessage(m, null, false);
                }
        ...
        protected MessageDTO preprocessValue(MessageDTO value, boolean fireEvent, boolean populate) {
            if (value != null && value.getPrimaryKey() != null && !value.getPrimaryKey().isDraft()) {
                ...
                if (!value.thread().hasValues()) {
                    value.thread().set(parentForm.getValue());
                }
            }
        ...
        public void saveMessage(MessageDTO m, boolean refresh) {
            ...
            if (p instanceof CommunicationEditorView.Presenter) {

                ((CommunicationEditorView.Presenter) p).saveMessage(new DefaultAsyncCallback<CommunicationThreadDTO>() {
                    @Override
                ...
                ((CommunicationViewerView.Presenter) p).saveMessage(m, null, refresh);
            }
        };

        protected void setCanReply(boolean canReply) {
            boolean isNew = getValue().isPrototype() || getValue().date() == null || getValue().date().isNull();
            btnReply.setVisible(canReply && getValue().thread().allowedReply().getValue(true) && !isNew);
        ...
        protected void onValueSet(boolean populate) {
            ...
            if (getValue().isPrototype() || getValue().date() == null || getValue().date().isNull()) {
                ...
                btnReply.setVisible(getValue().thread().allowedReply().getValue(true));
