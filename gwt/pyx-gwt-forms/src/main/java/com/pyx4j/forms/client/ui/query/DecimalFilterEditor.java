...
public class DecimalFilterEditor extends FilterEditorBase<IDecimalRangeCondition> {

    private static final I18n i18n = I18n.get(DecimalFilterEditor.class);

    private final DecimalBox fromBox;

    private final DecimalBox toBox;

    private final ValidationLabel fromBoxValidationLabel;

    private final ValidationLabel toBoxValidationLabel;

    public DecimalFilterEditor(IDecimalRangeCondition condition) {
        super(condition);
    ...
    public void save() throws ConditionInitializationException {
        if (!fromBox.isParsedOk() || !toBox.isParsedOk()) {
            ...
            getCondition().fromDecimal().setValue(fromBox.getValue());
            getCondition().toDecimal().setValue(toBox.getValue());
        }
    }
    ...
    class DecimalBox extends TextBox<Double> {
        DecimalBox() {

            setParser(new IParser<Double>() {
