...
public class DateGrid extends Grid {
	...
	protected DateGrid(Date minDate, Date maxDate, ArrayList<Date> disabledDates) {
		...
		setStyleName(Selector.getStyleName(DatePickerExtended.BASE_NAME,
				DatePickerExtended.StyleSuffix.Grid));
	...
	public void redraw(Date firstDisplayed) {
		...
		for (int i = 0; i < getNumCells(); i++) {
			...
			if (selectedDate != null
					&& CalendarUtil.isSameDate(lastDisplayed, selectedDate)) {
	...
	public DateCell getCell(Event e) {
		...
		return td != null ? elementToCell
				.get((com.google.gwt.user.client.Element) td) : null;
	}
