...
public abstract class MaintenanceRequestCategoryChoice<E extends MaintenanceRequestCategory> extends CEntityComboBox<E> {
    ...
    private MaintenanceRequestCategoryChoice<E> child;

    private MaintenanceRequestCategoryChoice<E> parent;
    ...
    public MaintenanceRequestCategoryChoice(Class<E> entityClass, Integer depth) {
        super(entityClass);
        this.depth = depth;
    ...
    public void assignParent(MaintenanceRequestCategoryChoice<E> parent, IObject<?> member) {
        if (parent == null) {
            return;
        }
    ...
    public void acceptChild(final MaintenanceRequestCategoryChoice<E> child) {
        this.child = child;
    }
    ...
    public void retriveOptions(final AsyncOptionsReadyCallback<E> callback) {
        ...
        if (crit != null) {
            ...
            if (depth == 0) {
                addCriterion(new PropertyCriterion(proto().parent(), Restriction.EQUAL, (Serializable) null));
            } else if (depth == 1) {
                addCriterion(new PropertyCriterion(proto().parent().parent(), Restriction.EQUAL, (Serializable) null));
            } else if (depth == 2) {
                addCriterion(new PropertyCriterion(proto().parent().parent().parent(), Restriction.EQUAL, (Serializable) null));
            } else if (depth == 3) {
                addCriterion(new PropertyCriterion(proto().parent().parent().parent().parent(), Restriction.EQUAL, (Serializable) null));
            }
        }
        resetOptions();
        super.retriveOptions(new AsyncOptionsReadyCallback<E>() {
            ...
            public void onOptionsReady(List<E> opt) {
                ...
                if (autoSelect) {
                    ...
                    Boolean test = opt != null && opt.size() > 0;
                    if (test) {
                        Boolean test2 = !isLeaf(opt.get(0));
                        Boolean test3 = test2;
                    }
