...
public class CreditCardValidationServiceImpl implements CreditCardValidationService {
    ...
    public void validate(AsyncCallback<CreditCardValidationResponce> callback, CreditCardInfo creditCardInfo) {
        CreditCardValidationResponce valid = ServerSideFactory.create(CreditCardFacade.class).validateCard(creditCardInfo);
        if (valid.validWithTypeProvided().getValue(false)) {
