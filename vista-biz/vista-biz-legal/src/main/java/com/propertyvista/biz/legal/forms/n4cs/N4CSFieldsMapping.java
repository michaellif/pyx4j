...
public class N4CSFieldsMapping extends LtbFormFieldsMapping<N4CSFormFieldsData> {
    ...
    protected void configure() {
        ...
        field(proto().issueDate()).mapTo("b12c96nfdate_served").define();
        ...
        mapping(proto().document(), new LtbFormFieldsMapping<N4CSDocumentType>(N4CSDocumentType.class) {
            @Override
            protected void configure() {
                field(proto().docType()).states("TT", "AA", "AH", "OS", "OR", "OO").mapTo("b12c96nfdocuments_served").define();
                field(proto().name()).mapTo("@b12c96nfdocuments_served").define();
            }
        });
        ...
        mapping(proto().passedTo(), new LtbFormFieldsMapping<N4CSToPersonInfo>(N4CSToPersonInfo.class) {
            @Override
            protected void configure() {
                field(proto().tpType()).states("Tenant", "Landlord", "Other").mapTo("b12c96nfdocuments_served").define();
                field(proto().name()).mapTo("b12c96nfChoice_4").define();
            }
        });
        ...
        mapping(proto().service(), new LtbFormFieldsMapping<N4CSServiceMethod>(N4CSServiceMethod.class) {
            @Override
            protected void configure() {
                field(proto().method()).states("H", "A", "P", "L", "D", "C", "F", "M", "O").mapTo("b12c96nfdocuments_served").define();
                field(proto().fax()).mapTo("b12c96nfChoice_9").define();
        ...
        mapping(proto().signature(), new LtbFormFieldsMapping<N4CSSignature>(N4CSSignature.class) {
            ...
            protected void configure() {
                ...
                field(proto().phone()).mapTo("b12c96nfap_phone_number").define();
                field(proto().signature()).mapTo("b12c96nmn4_signature").define();
                date(proto().signatureDate()).mapTo("b12c96nmsignature_date").define();
            }
        });
    }
}
