/*
 * (C) Copyright Property Vista Software Inc. 2011-2015 All Rights Reserved.
 *
 * This software is the confidential and proprietary information of Property Vista Software Inc. ("Confidential Information").
 * You shall not disclose such Confidential Information and shall use it only in accordance with the terms of the license agreement
 * you entered into with Property Vista Software Inc.
 *
 * This notice and attribution to Property Vista Software Inc. may not be removed.
 *
 * Created on Dec 11, 2014
 * @author vlads
 */
package com.propertyvista.biz.communication.template;

import java.util.List;

import com.pyx4j.entity.core.IEntity;
import com.pyx4j.entity.core.IObject;

import com.propertyvista.domain.communication.EmailTemplateType;

public class TemplateFacadeImpl implements TemplateFacade {

    @Override
    public <T extends IEntity> T getProto(EmailTemplateType type, Class<T> tClass) {
        return EmailTemplateManager.getProto(type, tClass);
    }

    @Override
    public String getVarname(IObject<?> member) {
        return EmailTemplateManager.getVarname(member);
    }

    @Override
    public List<String> getTemplateDataObjectSelection(EmailTemplateType template) {
        return EmailTemplateManager.getTemplateDataObjectSelection(template);
    }
}
