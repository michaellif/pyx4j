...
public class LeaseFacadeImpl implements LeaseFacade {
	...
	public Lease init(Lease lease) {
		...
		lease.leaseApplication().status()
				.setValue(LeaseApplication.Status.Created);
		...
		lease.billingAccount()
				.accountNumber()
				.setValue(
						ServerSideFactory.create(IdAssignmentFacade.class)
								.createAccountNumber());
	...
	public Lease setUnit(Lease lease, AptUnit unitId) {
		...
		AptUnit unit = Persistence.secureRetrieve(AptUnit.class,
				unitId.getPrimaryKey());
		Persistence.service().retrieve(unit.building());

		boolean succeeded = false;
		lease.unit().set(unit);

		EntityQueryCriteria<Service> criteria = new EntityQueryCriteria<Service>(
				Service.class);
		criteria.add(PropertyCriterion.eq(criteria.proto().catalog(), unit
				.building().productCatalog()));
		criteria.add(PropertyCriterion.eq(criteria.proto().version().type(),
				lease.type()));
		servicesLoop: for (Service service : Persistence.service().query(
				criteria)) {
			EntityQueryCriteria<ProductItem> serviceCriteria = EntityQueryCriteria
					.create(ProductItem.class);
			serviceCriteria.add(PropertyCriterion.eq(serviceCriteria.proto()
					.type(), ServiceItemType.class));
			serviceCriteria.add(PropertyCriterion.eq(serviceCriteria.proto()
					.product(), service.version()));
			serviceCriteria.add(PropertyCriterion.eq(serviceCriteria.proto()
					.element(), lease.unit()));
			ProductItem serviceItem = Persistence.service().retrieve(
					serviceCriteria);
			if (serviceItem != null) {
		...
		if (!succeeded) {
			...
			throw new UserRuntimeException(
					i18n.tr("There no service ''{0}'' for selected unit: {1} from Building: {2}",
							lease.type().getStringView(), unit.getStringView(),
							unit.building().getStringView()));
		}
	...
	public Lease setService(Lease lease, ProductItem serviceId) {
		...
		ProductItem serviceItem = Persistence.secureRetrieve(ProductItem.class,
				serviceId.getPrimaryKey());
		...
		if (!lease.unit().equals(serviceItem.element())) {
			throw new UserRuntimeException(
					i18n.tr("Invalid Unit/Service combination"));
		}
		...
		List<Deposit> deposits = ServerSideFactory.create(DepositFacade.class)
				.createRequiredDeposits(billableItem, node);
		if (deposits != null) {
		...
		if (serviceItem.product().getInstanceValueClass()
				.equals(Service.ServiceV.class)) {
			...
			if (feature.version().mandatory().isBooleanTrue()) {
				...
				for (ProductItem item : feature.version().items()) {
					if (item.isDefault().isBooleanTrue()) {
						...
						lease.version().leaseProducts().featureItems()
								.add(billableItem);
						deposits = createBillableItemDeposits(billableItem,
								node);
						if (deposits != null) {
							lease.billingAccount().deposits().addAll(deposits);
						}
	...
	public BillableItem createBillableItem(ProductItem itemId) {
		ProductItem item = Persistence.secureRetrieve(ProductItem.class,
				itemId.getPrimaryKey());
		assert item != null;
	...
	@Override
	public List<Deposit> createBillableItemDeposits(BillableItem item,
			PolicyNode node) {
		return ServerSideFactory.create(DepositFacade.class)
				.createRequiredDeposits(item, node);
	}
	...
	public Lease persist(Lease lease) {
		...
		if (lease.version().status().getValue() == Status.Application
				| lease.version().status().getValue() == Status.Created) {
			if (isNewLease) {
				doReserve = !lease.unit().isNull();
			} else {
				previousLeaseEdition = Persistence.secureRetrieve(Lease.class,
						lease.getPrimaryKey().asCurrentKey());

				if (!EqualsHelper.equals(previousLeaseEdition.unit()
						.getPrimaryKey(), lease.unit().getPrimaryKey())) {
		...
		if (doUnreserve) {
			switch (lease.version().status().getValue()) {
			case Application:
				ServerSideFactory.create(OccupancyFacade.class).unreserve(
						previousLeaseEdition.unit().getPrimaryKey());
				break;
			case Created:
				ServerSideFactory.create(OccupancyFacade.class).migratedCancel(
						previousLeaseEdition.unit()
								.<AptUnit> createIdentityStub());
				break;
			default:
				throw new IllegalStateException(
						SimpleMessageFormat
								.format("it's not allowed to unset unit while lease's state is \"{0}\"",
										lease.version().status().getValue()));
			}
		...
		if (doReserve) {
			switch (lease.version().status().getValue()) {
			case Application:
				ServerSideFactory.create(OccupancyFacade.class).reserve(
						lease.unit().getPrimaryKey(), lease);
				break;
			case Created:
				ServerSideFactory.create(OccupancyFacade.class).migrateStart(
						lease.unit().<AptUnit> createIdentityStub(), lease);
				break;
			default:
				throw new IllegalStateException(
						SimpleMessageFormat
								.format("it's not allowed to set unit while lease's state is \"{0}\"",
										lease.version().status().getValue()));
			}
		}
	...
	public Lease saveAsFinal(Lease lease) {
		...
		ServerSideFactory.create(ProductCatalogFacade.class)
	...
	private void persistCustomers(Lease lease) {
		for (Tenant tenant : lease.version().tenants()) {
			...
			if (!tenant.customer().isValueDetached()) {
				ServerSideFactory.create(CustomerFacade.class).persistCustomer(
						tenant.customer());
			}
		}
	...
	public void createMasterOnlineApplication(Key leaseId) {
		Lease lease = Persistence.retrieveDraftForEdit(Lease.class,
				leaseId.asDraftKey());
		...
		if (LeaseApplication.Status.Created != lease.leaseApplication()
				.status().getValue()) {
			throw new UserRuntimeException(i18n.tr("Invalid Application State"));
		}

		ServerSideFactory.create(OnlineApplicationFacade.class)
				.createMasterOnlineApplication(
						lease.leaseApplication().onlineApplication());

		lease.leaseApplication().status()
				.setValue(LeaseApplication.Status.OnlineApplication);
		lease.version().status().setValue(Lease.Status.Application);
		Persistence.service().persist(lease);
	}

	@Override
	public void approveApplication(Lease leaseId, Employee decidedBy,
			String decisionReason) {
		Lease lease = Persistence.secureRetrieveDraft(Lease.class,
				leaseId.getPrimaryKey());
		Persistence.service().retrieve(lease.version().tenants());

		Set<ValidationFailure> validationFailures = new LeaseApprovalValidator()
				.validate(lease);
		if (!validationFailures.isEmpty()) {
			...
			throw new UserRuntimeException(
					i18n.tr("This lease cannot be approved due to following validation errors:\n{0}",
							errorsRoster));
		}

		lease.version().status().setValue(Lease.Status.Approved);
		lease.leaseApplication().status()
				.setValue(LeaseApplication.Status.Approved);
		lease.approvalDate().setValue(
				new LogicalDate(Persistence.service()
						.getTransactionSystemTime()));
		lease.leaseApplication().decidedBy().set(decidedBy);
		lease.leaseApplication().decisionReason().setValue(decisionReason);
		lease.leaseApplication()
				.decisionDate()
				.setValue(
						new LogicalDate(Persistence.service()
								.getTransactionSystemTime()));
		...
		ServerSideFactory.create(OccupancyFacade.class).approveLease(
				lease.unit().getPrimaryKey());
		ServerSideFactory.create(ProductCatalogFacade.class)
				.updateUnitRentPrice(lease);
	...
	@Override
	public void declineApplication(Lease leaseId, Employee decidedBy,
			String decisionReason) {
		Lease lease = Persistence.secureRetrieveDraft(Lease.class,
				leaseId.getPrimaryKey());
		...
		lease.leaseApplication().status()
				.setValue(LeaseApplication.Status.Declined);
		lease.leaseApplication().decidedBy().set(decidedBy);
		lease.leaseApplication().decisionReason().setValue(decisionReason);
		lease.leaseApplication()
				.decisionDate()
	...
	@Override
	public void cancelApplication(Lease leaseId, Employee decidedBy,
			String decisionReason) {
		Lease lease = Persistence.secureRetrieveDraft(Lease.class,
				leaseId.getPrimaryKey());
		// TODO Review the status
		lease.version().status().setValue(Lease.Status.Closed);
		lease.leaseApplication().status()
				.setValue(LeaseApplication.Status.Cancelled);
		lease.leaseApplication().decidedBy().set(decidedBy);
		lease.leaseApplication().decisionReason().setValue(decisionReason);
		lease.leaseApplication()
				.decisionDate()
	...
	public void approveExistingLease(Lease leaseId) {
		...
		ServerSideFactory.create(OccupancyFacade.class).migratedApprove(
				lease.unit().<AptUnit> createIdentityStub());
		ServerSideFactory.create(ProductCatalogFacade.class)
				.updateUnitRentPrice(lease);
	...
	public void activate(Key leaseId) {
		...
		if (lease.billingAccount().carryforwardBalance().isNull()
				&& ServerSideFactory.create(BillingFacade.class)
						.getLatestBill(lease).billStatus().getValue() != Bill.BillStatus.Confirmed) {
			throw new UserRuntimeException(
					i18n.tr("Please run and confirm first bill in order to activate the lease."));
		}
		if (!EnumSet.of(Lease.Status.Created, Lease.Status.Approved).contains(
				lease.version().status().getValue())) {
			throw new UserRuntimeException(i18n.tr(
					"Impossible to activate lease with status: {0}", lease
							.version().status().getStringView()));
		}

		lease.version().status().setValue(Status.Active);
		lease.saveAction().setValue(SaveAction.saveAsFinal);
		lease.activationDate().setValue(
				new LogicalDate(Persistence.service()
						.getTransactionSystemTime()));
		Persistence.secureSave(lease);

		ServerSideFactory.create(UnitTurnoverAnalysisFacade.class)
				.propagateLeaseActivationToTurnoverReport(lease);

		// TODO move to LeadFacad
		// update Lead state (if present)
		EntityQueryCriteria<Lead> criteria = new EntityQueryCriteria<Lead>(
				Lead.class);
	...
	@Override
	public void createCompletionEvent(Key leaseId,
			CompletionType completionType, LogicalDate noticeDay,
			LogicalDate moveOutDay) {
		Lease lease = Persistence.secureRetrieveDraft(Lease.class, leaseId);
		if (lease == null) {
			throw new IllegalStateException("lease " + leaseId
					+ " was not found");
		}
		if (lease.version().status().getValue() != Status.Active) {
			throw new IllegalStateException("lease " + leaseId + " must be "
					+ Status.Active + " in order to perform Completion");
		}
		...
		ServerSideFactory.create(OccupancyFacade.class).endLease(
				lease.unit().getPrimaryKey());
	...
	public void cancelCompletionEvent(Key leaseId) {
		...
		if (lease.version().completion().isNull()) {
			throw new IllegalStateException("lease " + leaseId
					+ " must have notice in order to perform 'cancelNotice'");
		}
		...
		ServerSideFactory.create(OccupancyFacade.class).cancelEndLease(
				lease.unit().getPrimaryKey());
	}
