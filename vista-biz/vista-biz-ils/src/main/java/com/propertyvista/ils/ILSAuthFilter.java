...
/**
 * Uses Basic HTTP Authorization header in the form of <email>:<namespace>:<password> to authenticate the request
 */
public class ILSAuthFilter implements Filter {

    private final static Logger log = LoggerFactory.getLogger(ILSAuthFilter.class);
                        ...
                        if (credentials.length != 3) {
                            ...
                            LocalService.create(CrmAuthenticationService.class).authenticate(new AsyncCallback<AuthenticationResponse>() {
                                ...
                                public void onSuccess(AuthenticationResponse result) {
                                    rc.set(SecurityController.checkBehavior(VistaCrmBehavior.ILS_OLD));
