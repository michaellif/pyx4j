...
public class OnboardingMerchantAccountsLister extends BasicLister<OnboardingMerchantAccountDTO> {
    ...
    protected void onItemNew() {
        if (parentPmc != null && parentPmc.getPrimaryKey() != null) {
            AppSite.getPlaceController().goTo(new OperationsSiteMap.Management.OnboardingMerchantAccounts().formNewItemPlace(parentPmc.getPrimaryKey()));
        }
    }

    @Override
    protected void onItemSelect(OnboardingMerchantAccountDTO item) {
        AppSite.getPlaceController().goTo(new OperationsSiteMap.Management.OnboardingMerchantAccounts().formViewerPlace(item.getPrimaryKey()));
    }
