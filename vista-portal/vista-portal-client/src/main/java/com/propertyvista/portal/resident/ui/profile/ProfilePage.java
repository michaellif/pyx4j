            ...
            public BasicValidationError isValid() {
                ...
                if (!VistaFeatures.instance().yardiIntegration()) {
                    if (getCComponent().getValue().isEmpty()) {
                        return new BasicValidationError(getCComponent(), i18n.tr("Empty Emergency Contacts list"));
                    }
                }

                return !EntityGraph.hasBusinessDuplicates(getValue().emergencyContacts()) ? null : new BasicValidationError(getCComponent(), i18n
