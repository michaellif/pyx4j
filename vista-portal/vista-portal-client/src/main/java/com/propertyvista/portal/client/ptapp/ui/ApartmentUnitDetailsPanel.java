...
public class ApartmentUnitDetailsPanel extends FlowPanel 
	...
	private static I18n i18n = I18nFactory
			.getI18n(ApartmentUnitDetailsPanel.class);
	...
	public class GrowAnimation extends Animation {
		...
		@Override
		protected void onUpdate(final double progress) {
			panel.setHeight(String.valueOf((int) (progress * this.height))
					+ "px");
		}
	}
	...
	public class ShrinkAnimation extends Animation {
		...
		@Override
		protected void onUpdate(final double progress) {
			panel.setHeight(String
					.valueOf((int) ((1 - progress) * this.height)) + "px");
		}
	...
	public static final Type<AnimationCompleteEventHandler> TYPE = 
										new Type<AnimationCompleteEventHandler>();
    ...
    public HandlerRegistration addAnimationCompleteEventHandler(
            AnimationCompleteEventHandler handler) {
        return handlerManager.addHandler(TYPE, handler);
    }

	public void showUnitDetail(
			final ApartmentUnit unit,
			final MarketRent selectedmarketRent,
			final ValueChangeHandler<MarketRent> selectedMarketRentChangeHandler,
			IDebugId debugId) {

		this.clear();

		FlowPanel unitDetailPanel = new FlowPanel();
		unitDetailPanel.getElement().getStyle()
				.setVerticalAlign(VerticalAlign.TEXT_TOP);
		...
		amenitiesPanel.getElement().getStyle()
		...
		concessionPanel.getElement().getStyle()
		...
		Widget sp = new ViewLineSeparator(98, Unit.PCT, 0.5, Unit.EM, 0.5,
				Unit.EM);
		...
		for (final MarketRent mr : unit.marketRent()) {
			options.put(mr.leaseTerm().getValue(), mr.leaseTerm()
					.getStringView()
					+ i18n.tr(" month ")
					+ mr.rent().getStringView());
		}

		CRadioGroupInteger mrg = new CRadioGroupInteger(
				CRadioGroup.Layout.VERTICAL, options);
			...
			public void onValueChange(ValueChangeEvent<Integer> event) {
				for (final MarketRent mr : unit.marketRent()) {
					if (event.getValue().equals(mr.leaseTerm().getValue())) {
						selectedMarketRentChangeHandler
								.onValueChange(new ValueChangeEvent<MarketRent>(
										mr) {
								});
						break;
					}
		...
		HTML availabilityAndPricing = new HTML(SiteResources.INSTANCE
				.availabilityAndPricing().getText());
	...
	public void hide() {
		...
		if(this.getWidgetCount() > 0){
			...
			new ShrinkAnimation(this, panel).run(500);
		}else{
			handlerManager.fireEvent(new AnimationCompleteEvent());			
		}
	}
