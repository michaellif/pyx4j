...
public class ApplicationConfirmationForm extends CPortalEntityForm<PaymentMethodDTO> {
    ...
    public IsWidget createContent() {
        ...
        int row = -1;
        mainPanel.setWidget(++row, 0, new FormWidgetDecoratorBuilder(inject(proto().paymentMethod(), new CEntityLabel<LeasePaymentMethod>()), 250)
    ...
    private Widget createAutoPaySignupPanel() {
        ...
        text.add(new Anchor(i18n.tr("Sign up for Auto Pay today"), new Command() {
            @Override
            public void execute() {
                ((ApplicationConfirmationPresenter) getView().getPresenter()).goToAutoPay();
            }
        }));
