...
public class PaymentWizard extends CPortalEntityWizard<PaymentDTO> {
        ...
        get(proto().amount()).addComponentValidator(new AbstractComponentValidator<BigDecimal>() {
            ...
            public BasicValidationError isValid() {
                if (getCComponent().getValue() != null) {
                    return (getCComponent().getValue().signum() > 0 ? null : new BasicValidationError(getCComponent(), i18n
    ...
    private Widget createDirectBankingPanel() {
        ...
        panel.add(createDecorator(i18n.tr("Account Number:"), getValue().billingAccount().accountNumber().getStringView()));
