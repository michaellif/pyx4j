...
public class TenantSurePaymentViewer extends CViewer<TenantSurePaymentDTO> {
    ...
    public TenantSurePaymentViewer(final NumberFormat currencyFormat, final DateTimeFormat dateFormat) {

        setFormatter(new IFormatter<TenantSurePaymentDTO, IsWidget>() {

            @Override
            public IsWidget format(TenantSurePaymentDTO payment) {
                ...
                if (payment != null && !payment.isNull()) {
                    ...
                    for (TenantSurePaymentItemDTO paymentItem : payment.paymentBreakdown()) {
                        ...
                        new ViewerRecordWriter(currencyFormat).addDetailRecord(paymentBreakdownPanel, ++innerRow, paymentItem.description().getValue(),
                                paymentItem.amount().getValue());
                        for (TenantSurePaymentItemTaxDTO tax : paymentItem.taxBreakdown()) {
                            new ViewerRecordWriter(currencyFormat).addDetailRecord(paymentBreakdownPanel, ++innerRow, tax.tax().getValue(), tax.amount()
                                    .getValue());
                        }
                    }
                    if (!payment.total().isNull()) {
                        new ViewerRecordWriter(currencyFormat).addTotalRecord(paymentBreakdownPanel, ++innerRow, payment.total().getMeta().getCaption(),
                                payment.total().getValue());
                    }
