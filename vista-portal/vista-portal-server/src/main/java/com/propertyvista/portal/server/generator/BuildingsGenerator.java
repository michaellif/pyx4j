...
public class BuildingsGenerator {
	...
	private final static Logger log = LoggerFactory
			.getLogger(BuildingsGenerator.class);
	...
	private static String resourceFileName(String fileName) {
		return BuildingsGenerator.class.getPackage().getName()
				.replace('.', '/')
				+ "/" + fileName;
	}
	...
	public Building createBuilding(int b) {
		// building type
		BuildingInfo.Type buildingType = RandomUtil.random(BuildingInfo.Type
				.values());
		...
		Building building = createBuilding(propertyCode, buildingType, complex,
				website, address, phones, email);
	...
	public List<AptUnit> createUnits(Building building,
			List<Floorplan> floorplans, int numFloors, int numUnitsPerFloor) {
		...
		for (int floor = 1; floor < numFloors + 1; floor++) {
			...
			for (int j = 1; j < numUnitsPerFloor + 1; j++) {
				...
				int uarea = floorplan.minArea().getValue()
						+ RandomUtil.randomInt(10);
				AptUnit unit = createUnit(building, suiteNumber, floor, uarea,
						bedrooms, bathrooms, floorplan);
				units.add(unit);
			}
		}
		return units;
	...
	private Building createBuilding(String propertyCode,
			BuildingInfo.Type buildingType, Complex complex, String website,
			Address address, List<Phone> phones, Email email) {
		...
		building.marketing()
				.name()
				.setValue(
						RandomUtil.randomLetters(4) + " "
								+ RandomUtil.randomLetters(6));
	...
	public static Concession createConcession(Concession.AppliedTo appliedTo,
			double months, double percentage) {
	...
	private AptUnit createUnit(Building building, String suiteNumber,
			int floor, double area, double bedrooms, double bathrooms,
			Floorplan floorplan) {
		...
		unit.info().type()
				.setValue(RandomUtil.random(AptUnitInfo.Type.values()));
		...
		unit.financial().marketRent()
				.setValue(900. + RandomUtil.randomInt(200));
		...
		if (RandomUtil.randomBoolean()) {
			unit.amenities().add(
					createUnitAmenity(RandomUtil.random(AptUnitAmenity.Type
							.values())));
		}
		if (RandomUtil.randomBoolean()) {
			unit.amenities().add(
		...
		if (RandomUtil.randomBoolean()) {
			unit.info()
					.details()
					.add(createUnitDetailItem(RandomUtil
							.random(AptUnitItem.Type.values())));
		} else {
			unit.info()
					.details()
					.add(createUnitDetailItem(RandomUtil
		...
		// concessions
		if (RandomUtil.randomBoolean()) {
			unit.concessions().add(
					createConcession(
							RandomUtil.random(Concession.AppliedTo.values()),
							1.0 + RandomUtil.randomInt(3), 0));
		}
		if (RandomUtil.randomBoolean()) {
			unit.concessions().add(
					createConcession(
							RandomUtil.random(Concession.AppliedTo.values()),
							1.0 + RandomUtil.randomInt(11), 15.8));
		}
		...
		unit.avalableForRent().setValue(
				new java.sql.Date(avalable.getTime().getTime()));
	...
	public static PropertyProfile createPropertyProfile(int index) {
		PropertyProfile propertyProfile = EntityFactory
				.create(PropertyProfile.class);
	...
	public LeaseTerms createLeaseTerms() {
		...
		try {
			leaseTerms
					.text()
					.setValue(
							IOUtils.getTextResource(resourceFileName("leaseTerms.html")));
		} catch (IOException e) {
			throw new Error(e);
		}
		return leaseTerms;
