...
public class PmcDocumentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            PmcDocumentFile docFile = Persistence.service().retrieve(PmcDocumentFile.class, parseKey(request.getPathInfo()));
            if (docFile == null) {
                throw new FileNotFoundException("document file was not found");
            }
            assertPermission(docFile);

            PmcDocumentBlob blob = Persistence.service().retrieve(PmcDocumentBlob.class, docFile.blobKey().getValue());
            if (blob == null) {
    ...
    private void assertPermission(PmcDocumentFile docFile) {
