...
public class ValueAdapterDouble extends ValueAdapterPrimitive {

    protected ValueAdapterDouble(Dialect dialect) {
        super(dialect, Double.class);
    }
    ...
    public int bindValue(PreparedStatement stmt, int parameterIndex, IEntity entity, MemberOperationsMeta member) throws SQLException {
        Double value = (Double) member.getMemberValue(entity);
        if (value == null) {
    ...
    public void retrieveValue(ResultSet rs, IEntity entity, MemberOperationsMeta member) throws SQLException {
        double value = rs.getDouble(member.sqlName());
        if (rs.wasNull()) {
