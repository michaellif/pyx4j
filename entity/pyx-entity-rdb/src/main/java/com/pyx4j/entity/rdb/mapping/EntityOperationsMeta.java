                ...
                if (ICollection.class.isAssignableFrom(memberMeta.getObjectClass())) {
                    ...
                    if (joinInfo == null) {
                        ...
                        member = new MemberCollectionOperationsMeta(memberAccess, valueAdapter, sqlName, memberMeta, path + Path.PATH_SEPARATOR + memberName
                                + Path.PATH_SEPARATOR, null, false, namingConvention.sqlAutoGeneratedJoinOwnerColumnName(), ownerValueAdapter,
                                namingConvention.sqlAutoGeneratedJoinValueColumnName(), namingConvention.sqlAutoGeneratedJoinOrderColumnName(), true);
                    } else {
                        member = new MemberCollectionOperationsMeta(memberAccess, valueAdapter, joinInfo.sqlName, memberMeta, path + Path.PATH_SEPARATOR
                                ...
                                joinInfo.ownerValueAdapter, joinInfo.sqlValueName, joinInfo.sqlOrderColumnName, false);
                    }
                    ...
                    MemberCollectionOperationsMeta member = new MemberCollectionOperationsMeta(memberAccess, valueAdapter, sqlName, memberMeta, path
                            + Path.PATH_SEPARATOR + memberName + Path.PATH_SEPARATOR, null, false, namingConvention.sqlAutoGeneratedJoinOwnerColumnName(),
                            ownerValueAdapter, namingConvention.sqlAutoGeneratedJoinValueColumnName(), null, true);
                    collectionMembers.add(member);
                    membersByPath.put(member.getMemberPath(), member);
                    allMembers.add(member);
                } else {
