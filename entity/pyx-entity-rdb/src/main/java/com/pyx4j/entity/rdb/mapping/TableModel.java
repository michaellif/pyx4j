...
public class TableModel {
    ...
    private static boolean ddlLogVersionOnce = true;
    ...
    public boolean ensureExists(PersistenceContext persistenceContext, Ddl ddl) throws SQLException {
        ...
        try {
            ...
            logApplicationVersionOnce();
            ddlLog.error("DDL operation error", e);
            throw e;
        }
    ...
    private void logApplicationVersionOnce() {
        if (ddlLogVersionOnce) {
            ddlLog.info("Application version {}", ApplicationVersion.getBuildLabel());
            ddlLogVersionOnce = false;
        }
    }
    ...
    public int enabelForeignKeys(PersistenceContext persistenceContext, boolean enable) throws SQLException {
        int count = 0;
        List<String> sqls = new Vector<String>();
        for (MemberOperationsMeta member : entityOperationsMeta.getAutogeneratedCollectionMembers()) {
            TableMetadata memberTableMetadata = TableMetadata.getTableMetadata(persistenceContext, mappings.getConfiguration(), member.sqlName());
            if (memberTableMetadata != null) {
                memberTableMetadata.readReferenceForeignKeys(persistenceContext);
                for (String fk : memberTableMetadata.getForeignKeyNames()) {
                    sqls.add(persistenceContext.getDialect().enabelConstraint(member.sqlName(), fk, enable));
                    count++;
                }
                for (Map.Entry<String, String> ref : memberTableMetadata.getForeignKeysReference().entrySet()) {
                    sqls.add(persistenceContext.getDialect().enabelConstraint(ref.getValue(), ref.getKey(), enable));
                    count++;
                }
            }
        }
        ...
        if (memberTableMetadata != null) {
            memberTableMetadata.readReferenceForeignKeys(persistenceContext);
            for (String fk : memberTableMetadata.getForeignKeyNames()) {
                sqls.add(persistenceContext.getDialect().enabelConstraint(tableName, fk, enable));
                count++;
            }
            for (Map.Entry<String, String> ref : memberTableMetadata.getForeignKeysReference().entrySet()) {
                sqls.add(persistenceContext.getDialect().enabelConstraint(ref.getValue(), ref.getKey(), enable));
    ...
    private String sqlInsert() {
        if (sqlInsert == null) {
            ...
            SQLExpressionListBuilder sqlColumns = new SQLExpressionListBuilder();
            SQLExpressionListBuilder sqlValues = new SQLExpressionListBuilder();

            if (classModel != ModelType.regular) {
                sqlColumns.add(dialect.sqlDiscriminatorColumnName());
                sqlValues.add("?");
            }

            for (MemberOperationsMeta member : entityOperationsMeta.getColumnMembers()) {
                ValueAdapter valueAdapter = member.getValueAdapter();
                for (String name : valueAdapter.getColumnNames(member.sqlName())) {
                    sqlColumns.add(name);
                    sqlValues.add(valueAdapter.toSqlValue(dialect, name, "?"));
                }
            }

            for (MemberOperationsMeta member : entityOperationsMeta.getIndexMembers()) {
                sqlColumns.add(member.sqlName());
                sqlValues.add("?");
            }
            if (dialect.isMultitenantSharedSchema()) {
                sqlColumns.add(dialect.getNamingConvention().sqlNameSpaceColumnName());
                sqlValues.add("?");
            }

            if (!returnAutoGeneratedKeys) {
                sqlColumns.add(dialect.getNamingConvention().sqlIdColumnName());
                sqlValues.add("?");
            ...
            if (returnAutoGeneratedKeys && dialect.isSequencesBaseIdentity()) {
                ...
                sqlValues.add(dialect.sqlSequenceNextVal(sequenceName));
            }

            sqlInsert = " (" + sqlColumns.toString() + ") VALUES (" + sqlValues.toString() + ")";
        }
        return sqlInsert;
    }
        ...
        if (sqlUpdate == null) {
            ...
            for (MemberOperationsMeta member : entityOperationsMeta.getColumnMembers()) {
                ...
                for (String name : valueAdapter.getColumnNames(member.sqlName())) {
                    ...
                    sql.append(valueAdapter.toSqlValue(dialect, name, "?"));
                    sql.append(" ");
                }
            }
            for (MemberOperationsMeta member : entityOperationsMeta.getIndexMembers()) {
        ...
        for (MemberOperationsMeta member : entityOperationsMeta.getColumnMembers()) {
            if (member.getMemberMeta().isEntity()) {
                ...
                if ((childEntity.getPrimaryKey() == null) && childEntity.hasValues()) {
                    log.error("Saving non persisted reference {}\n{}\n", childEntity, PersistenceTrace.getCallOrigin());
        ...
        try {
            ...
            if (encodedCursorReference != null) {
                if (limitCriteriatApplied) {
                    log.error("page restriction present, encodedCursorReference {} ignored", encodedCursorReference);
                } else {
                    offset = Integer.valueOf(encodedCursorReference);
