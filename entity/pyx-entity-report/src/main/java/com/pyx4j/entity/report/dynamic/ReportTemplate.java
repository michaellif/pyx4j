...
public class ReportTemplate {
    ...
    static {
        ...
        columnTitleStyle = stl.style(columnStyle).setBorder(stl.pen1Point()).setHorizontalAlignment(HorizontalAlignment.CENTER)
        ...
        reportTemplate = template().setLocale(Locale.ENGLISH).setColumnStyle(columnStyle).setColumnTitleStyle(columnTitleStyle).setGroupStyle(groupStyle)
                .setGroupTitleStyle(groupStyle).setSubtotalStyle(subtotalStyle).highlightDetailEvenRows().crosstabHighlightEvenRows()
    ...
    public static ComponentBuilder<?, ?> reportTitleBuilder(String link, String imageResourcePath, String reportTitle) {
        return cmp.horizontalList(cmp.image(Templates.class.getResource(imageResourcePath)).setFixedDimension(60, 60),
                cmp.verticalList(cmp.text(reportTitle).setStyle(bold16CenteredStyle).setHorizontalAlignment(HorizontalAlignment.JUSTIFIED),
