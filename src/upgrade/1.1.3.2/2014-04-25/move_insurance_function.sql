...
BEGIN
    ...
    **/
    ...
    ALTER TABLE payment_payment_details ADD COLUMN old_id BIGINT;
    ALTER TABLE payment_method ADD COLUMN old_id BIGINT;
    ALTER TABLE tenant_sure_insurance_policy_client ADD COLUMN old_id BIGINT;
    
    
    EXECUTE 'INSERT INTO '||v_new_schema||'.payment_payment_details (id,old_id,id_discriminator,'
    ...
    EXECUTE 'INSERT INTO '||v_new_schema||'.payment_method(id,old_id,tenant,details,id_discriminator,payment_type,'
            ...
            ||'JOIN    '||v_new_schema||'.payment_payment_details ppd ON (ppd.old_id = t.details) '
            ||'JOIN    '||v_new_schema||'.lease_participant lp ON (lp.participant_id = t.participant_id) '
            ||'JOIN    '||v_new_schema||'.province p ON (p.code = t.code) '
            ||'JOIN    '||v_new_schema||'.country c ON (c.name = t.name))';
    
    ALTER TABLE insurance_policy ADD COLUMN old_id BIGINT;
    ALTER TABLE insurance_certificate ADD COLUMN old_id BIGINT;
    ALTER TABLE insurance_certificate_scan_blob ADD COLUMN old_id BIGINT;
    
    EXECUTE 'INSERT INTO '||v_new_schema||'.tenant_sure_insurance_policy_client (id,old_id,tenant_discriminator,'
            ...
            ||'JOIN    '||v_new_schema||'.lease_participant lp ON (t.participant_id = lp.participant_id))';
    
    EXECUTE 'INSERT INTO '||v_new_schema||'.insurance_policy (id,old_id,tenant,id_discriminator,tenant_discriminator,'
            ...
            ||'signature,broker_fee) '
            ||'(SELECT  nextval(''public.insurance_policy_seq'') AS id,t.id AS old_id,'
            ||'         lp.id AS tenant,t.id_discriminator,t.tenant_discriminator,'
            ||'         t.is_deleted,ts.id AS client,t.quote_id,t.status,t.cancellation,'
            ||'         t.cancellation_description_reason_from_tenant_sure,'
            ||'         t.payment_day,t.cancellation_date,t.annual_premium,'
            ||'         t.underwriter_fee,t.total_annual_tax,t.total_annual_payable,'
            ||'         t.total_first_payable,t.total_anniversary_first_month_payable,'
            ||'         t.total_monthly_payable,t.contents_coverage,t.deductible,t.payment_schedule,'
            ||'         t.signature,t.broker_fee '
            ||'FROM    _dba_.insurance_policy t '
            ||'JOIN    '||v_new_schema||'.lease_participant lp ON (lp.participant_id = t.participant_id) '
            ||'LEFT JOIN   '||v_new_schema||'.tenant_sure_insurance_policy_client ts ON (ts.old_id = t.client))';
    
    
    EXECUTE 'INSERT INTO '||v_new_schema||'.insurance_certificate (id,old_id,id_discriminator,'
            ||'insurance_policy_discriminator,insurance_policy,is_managed_by_tenant,insurance_provider,'
            ...
            ||'JOIN    '||v_new_schema||'.insurance_policy i ON (i.old_id = t.insurance_policy))';
    
    
    EXECUTE 'INSERT INTO '||v_new_schema||'.insurance_certificate_scan_blob (id,old_id,content_type,'
    ...
    EXECUTE 'INSERT INTO '||v_new_schema||'.insurance_certificate_scan (id,file_file_name,'
            ||'file_updated_timestamp,file_cache_version,file_file_size,file_content_mime_type,'
            ||'file_blob_key,description,certificate,certificate_discriminator) '
            ...
            ||'JOIN    '||v_new_schema||'.insurance_certificate ic ON (ic.old_id = t.certificate) '
            ||'JOIN    '||v_new_schema||'.insurance_certificate_scan_blob b ON (b.old_id = t.file_blob_key))'; 
    
    EXECUTE 'INSERT INTO '||v_new_schema||'.tenant_sure_transaction(id,insurance_discriminator,'
            ...
            ||'JOIN    '||v_new_schema||'.payment_method pm ON (pm.old_id = t.payment_method))';
    
    EXECUTE 'INSERT INTO '||v_new_schema||'.tenant_sure_insurance_policy_report(id,insurance_discriminator,'
    ...
    ALTER TABLE insurance_certificate_scan_blob DROP COLUMN old_id;
    ALTER TABLE payment_payment_details DROP COLUMN old_id;
    ALTER TABLE payment_method DROP COLUMN old_id;
    ALTER TABLE tenant_sure_insurance_policy_client DROP COLUMN old_id;
