...
public class FormBasicTest extends TesterSeleniumTestBase {
    ...
    public void testTextBox() {
        ...
        selenium.click(D.id(proto(CComponentProperties.class).mandatory()));

        // Check mandatory asterisk" visible
        assertVisible(D.id(D.id(proto(EntityI.class).textBox()), WidgetDecorator.DebugIds.MandatoryImage.debugId()));

        // Click again
        selenium.click(D.id(proto(CComponentProperties.class).mandatory()));

        // Check mandatory "asterisk" disappeared
        assertNotVisible(D.id(D.id(proto(EntityI.class).textBox()), WidgetDecorator.DebugIds.MandatoryImage.debugId()));
        ...
        selenium.click(D.id(proto(CComponentProperties.class).visible()));

        // Check that component invisible
        assertNotVisible(D.id(proto(EntityI.class).textBox()));

        // Set visible
        selenium.click(D.id(proto(CComponentProperties.class).visible()));

        // Check that component visible
        assertVisible(D.id(proto(EntityI.class).textBox()));
    }
}
