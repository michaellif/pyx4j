/*
 * (C) Copyright Property Vista Software Inc. 2011-2012 All Rights Reserved.
 *
 * This software is the confidential and proprietary information of Property Vista Software Inc. ("Confidential Information"). 
 * You shall not disclose such Confidential Information and shall use it only in accordance with the terms of the license agreement 
 * you entered into with Property Vista Software Inc.
 *
 * This notice and attribution to Property Vista Software Inc. may not be removed.
 *
 * Created on Dec 27, 2013
 * @author vlads
 * @version $Id$
 */
package com.propertyvista.server.security.openId;

import java.util.Date;

import org.openid4java.consumer.InMemoryNonceVerifier;

import com.propertyvista.biz.system.WorldDateManager;

public class TimeShiftInMemoryNonceVerifier extends InMemoryNonceVerifier {

    @Override
    protected boolean isTooOld(Date now, Date nonce) {
        return super.isTooOld(WorldDateManager.getWorldTime(), nonce);
    }

    @Override
    protected int seen(Date now, String opUrl, String nonce) {
        return super.seen(WorldDateManager.getWorldTime(), opUrl, nonce);
    }
}
