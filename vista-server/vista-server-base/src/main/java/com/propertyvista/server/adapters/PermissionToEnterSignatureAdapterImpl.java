...
import com.propertyvista.domain.security.PermissionToEnterSignature;
import com.propertyvista.shared.adapters.PermissionToEnterSignatureAdapter;

public class PermissionToEnterSignatureAdapterImpl implements PermissionToEnterSignatureAdapter {

    @Override
    public void onBeforeUpdate(PermissionToEnterSignature origEntity, PermissionToEnterSignature newEntity) {
        if (newEntity.agree().getValue(false)) {
            if ((origEntity == null) || !origEntity.agree().getValue(false)) {
                // TODO - allow signing in CRM?
                if (!SignatureFormat.AgreeBox.equals(newEntity.signatureFormat().getValue()) && //
                        !SignatureFormat.None.equals(newEntity.signatureFormat().getValue())) {
