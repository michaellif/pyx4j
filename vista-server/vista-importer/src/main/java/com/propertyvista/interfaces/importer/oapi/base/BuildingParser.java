...
public class BuildingParser {
    ...
    private final static Logger log = LoggerFactory.getLogger(BuildingParser.class);

    private static final I18n i18n = I18n.get(BuildingParser.class);

    private final List<BuildingModel> buildings = new ArrayList<BuildingModel>();

    public List<BuildingModel> parseBuildings(byte[] data, DownloadFormat format) {
        if (format == DownloadFormat.XLS || format == DownloadFormat.XLSX) {
            return parseExcelBuildings(data, format);
        } else {
            throw new IllegalArgumentException();
        }
    }

    private List<BuildingModel> parseExcelBuildings(byte[] data, DownloadFormat format) {
        if ((format != DownloadFormat.XLS) && (format != DownloadFormat.XLSX)) {
            throw new IllegalArgumentException();
        }
        XLSLoad loader;
        ...
        for (int sheetNumber = 0; sheetNumber < sheets; sheetNumber++) {
            ...
            EntityCSVReciver<BuildingModel> receiver = new BuildingCSVReciver(loader.getSheetName(sheetNumber));
            try {
            ...
            buildings.addAll(receiver.getEntities());
        }
        return buildings;

    }

    public static class BuildingCSVReciver extends EntityCSVReciver<BuildingModel> {
        String sheetNumber;

        public BuildingCSVReciver(String sheetName) {
            super(BuildingModel.class);
        ...
        @Override
        public void onRow(BuildingModel entity) {
            if (!entity.isNull()) {
