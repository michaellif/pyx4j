...
public abstract class AbstractDeferredProcess implements IDeferredProcess {
    ...
    public static class RunningProcess implements Serializable {
        ...
        public final AtomicReference<String> completionMessage = new AtomicReference<>();

        public final void setCompletionMessage(String message) {
    ...
    public DeferredProcessProgressResponse status() {
        ...
        if (completed) {
            r.setCompleted();
            r.setMessage(progress.completionMessage.get());
        } else if (canceled) {
            r.setCanceled();
        } else if (!started) {
            r.setQueued();
